...
const checks: Check[] = [
  { name: "java", command: "java", args: ["-version"], required: true, note: "required by jadx/apktool/baksmali" },
  {
    ...
    args: ["version"],
    required: true,
    note: "fast manifest/package/activity extraction",
  },
  { name: "aapt2", command: "aapt2", args: ["version"], required: false, note: "modern Android build-tools fallback" },
  {
    ...
    args: ["--help"],
    required: true,
    note: "official manifest/APK analysis",
  },
  ...
  { name: "apktool", command: "apktool", args: ["--version"], required: true, note: "resource/smali decode fallback" },
  {
    ...
    args: ["--version"],
    required: false,
    note: "dex/smali focused disassembly",
  },
  {
    name: "smali",
    command: "smali",
    args: ["--version"],
    required: false,
    note: "smali assembly when patching test copies",
  },
...
if (missingRequired.length) {
  console.log(
    "Install or PATH-fix missing_required tools before expecting APK fast-path performance. Until then, ctf-apk-triage will fall back to ZIP/dex/native string scanning where possible.",
  )
} else {
  console.log(
    "Android APK fast-path tooling is ready for static triage. Run ctf-android-runtime-check separately for device/frida readiness.",
  )
}
