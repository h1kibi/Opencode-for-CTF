...
function score(card: Card, query: string) {
  ...
  const hay =
    `${card.title} ${(card.query_aliases || []).join(" ")} ${(card.semantic_tokens || []).join(" ")} ${(card.retrieval_intents || []).join(" ")} ${card.trigger}`.toLowerCase()
  let s = 0
...
for (const query of QUERY_SET) {
  const hits = cards
    .map((card) => ({ id: card.id, title: card.title, score: score(card, query) }))
