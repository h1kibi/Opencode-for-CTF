...
async function main() {
  ...
  lines.push(await expectThrows(
    "ctf-safe-extract-missing-target",
    () => ctfSafeExtract.execute({ target: "", out: "extracted", maxFiles: 10, maxBytes: 1024 * 1024, overwrite: false, flagPattern: "" }, context as any),
    /target is required/i,
  ))

  lines.push(await expectThrows(
    "archive-safe-extract-missing-target",
    () => archiveSafeExtract.execute({ target: "", out: "extracted", maxFiles: 10, maxBytes: 1024 * 1024, overwrite: false }, context as any),
    /target is required/i,
  ))
  ...
  try {
    ...
    const ctfOut = String(await ctfSafeExtract.execute({
    ...
    const archiveOut = String(await archiveSafeExtract.execute({
      target: "single-file.zip",
      out: "out",
      maxFiles: 50,
      maxBytes: 1024 * 1024,
    ...
    assert(extractedPath, `root-level zip extraction should contain flag.txt under ${path.join(extractRoot, "single-file")}`)
    ...
    assert(/status: extracted|verdict: direct_flag|verdict: archive_extracted/i.test(ctfOut), `ctf-safe-extract unexpected output: ${ctfOut}`)
  ...
  assert(baselineStatus === "connection_error", `remote fingerprint baseline status should be connection_error, got ${baselineStatus}`)
  assert(mutantStatus === "connection_error", `remote fingerprint mutant status should be connection_error, got ${mutantStatus}`)
