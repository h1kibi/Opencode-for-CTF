...
function i32(buf: Buffer, off: number) { return off + 4 <= buf.length ? buf.readInt32LE(off) : 0 }
...
export default tool({
  description: "CTF reverse PE slice: extract compact PE metadata, suspicious imports/strings, sections, and keyword-focused disassembly slices without dumping huge outputs.",
  args: {
    target: tool.schema.string().describe("PE file path to inspect"),
    keyword: tool.schema.string().optional().describe("Regex keyword for focused string/disasm slices. Default malware/rev high-signal keywords."),
  ...
  async execute(args, context) {
    ...
    if (e_lfanew + 0x108 > buf.length || buf.subarray(e_lfanew, e_lfanew + 4).toString() !== "PE\0\0") throw new Error("invalid PE header")
    ...
    const sections: Array<{ name: string; vsize: number; vaddr: number; rawSize: number; rawPtr: number; flags: string; entropy: number }> = []
    for (let i = 0; i < numberOfSections; i++) {
      ...
      const raw = rawPtr && rawSize && rawPtr + rawSize <= buf.length ? buf.subarray(rawPtr, rawPtr + rawSize) : Buffer.alloc(0)
      sections.push({ name, vsize, vaddr, rawSize, rawPtr, flags: sectionFlags(chars), entropy: Number(entropy(raw).toFixed(3)) })
    }
    ...
    const objdumpHeaders = await tryExec("objdump", ["-x", target], path.dirname(target), 12000)
    const importLines = objdumpHeaders.split(/\r?\n/).filter((line) => kw.test(line) || /DLL Name:|Ordinal\/Hint|Import Address Table/i.test(line))
    const suspiciousImports = importLines.filter((line) => kw.test(line)).slice(0, 80)

    const disasm = await tryExec("objdump", ["-d", target], path.dirname(target), 20000)
    ...
    return [
      ...
      "sections:",
      ...sections.slice(0, 24).map((s) => `- ${s.name}\tvaddr=0x${s.vaddr.toString(16)}\traw=0x${s.rawPtr.toString(16)}\tsize=${s.rawSize}\tflags=${s.flags || "none"}\tentropy=${s.entropy}`),
      "suspicious_imports:",
