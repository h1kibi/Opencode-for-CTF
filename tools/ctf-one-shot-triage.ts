...
const SKIP_DIRS = new Set([".git", "node_modules", "dist", "build", ".next", "__pycache__", "venv", ".venv", "target", ".cache", "coverage"])
...
function priorityScore(rel: string) {
  ...
  let score = 0
  if (/^(readme|dockerfile|docker-compose\.ya?ml|compose\.ya?ml|package\.json|requirements\.txt|pyproject\.toml|pom\.xml|build\.gradle|go\.mod|cargo\.toml|composer\.json)$/i.test(base)) score += 24
...
function safeDirectFlagCandidates(text: string, sample: Buffer, flagRegex: RegExp) {
  ...
  const binaryLike = sample.includes(0x00) || sample.subarray(0, 4).equals(Buffer.from([0x7f, 0x45, 0x4c, 0x46])) || sample.subarray(0, 2).toString() === "MZ"
...
function signalFromFile(file: string, rel: string, sample: Buffer, text: string, strings: string[], flagRegex: RegExp): Signal[] {
  const lower = `${rel}\n${text.slice(0, 30000)}\n${strings.join("\n")}`.toLowerCase()
  ...
  for (const flag of flagCandidates.suspicious) add("binary_flag_like_text", `binary-like sample contains flag-like text: ${flag}`, 42)

  if (/\.(zip|jar|war|apk|docx|xlsx|pptx)$/i.test(rel) || magic.startsWith("50 4b")) add("archive", "zip-like archive; use ctf-safe-extract", 90)
  else if (/\.7z$/i.test(rel) || magic.startsWith("37 7a bc af 27 1c")) add("archive", "7z archive; use ctf-safe-extract", 88)
  else if (/(\.tar|\.tar\.gz|\.tgz|\.gz|\.bz2|\.xz)$/i.test(rel)) add("archive", "tar/compressed archive; use ctf-safe-extract", 84)
  if (/(?:^|[\\/])(chall|challenge|pwn|vuln|rop|fmt|heap|baby|main|bin|elf)[^\\/]*\.(zip|jar|7z|rar|tar|tgz|gz)$/i.test(rel)) add("pwn_archive_name", "archive name looks like a packaged native challenge", 74)

  if (/\b(n|e|c|p|q|dp|dq|phi)\d*\b\s*[:=]\s*(0x[0-9a-fA-F]+|\d+)/.test(text) || /BEGIN (RSA )?(PUBLIC|PRIVATE) KEY/.test(text)) add("rsa", "RSA-like parameters/key material", 82)
  if (/sourceMappingURL=|\.map$/i.test(`${rel}\n${text.slice(0, 12000)}`)) add("source_map", "source map indicator", 80)
  if (/pom\.xml|build\.gradle|spring|servlet|jsessionid|tomcat|jsp|@getmapping|@postmapping|@requestmapping/i.test(`${rel}\n${text.slice(0, 25000)}`)) add("java_web", "Java/Spring/Servlet web source indicator", 78)
  if (/openapi|swagger|"paths"\s*:|\b(GET|POST|PUT|PATCH|DELETE)\s+\/api\//.test(text)) add("api_map", "OpenAPI/Swagger/plain API list indicator", 76)
  if (/app\.py|server\.(js|ts)|index\.php|routes?|controllers?|templates?|dockerfile|package\.json|requirements\.txt|flask|express|django|fastapi|laravel|graphql|jwt|cookie|session|csrf|upload|admin/i.test(lower)) add("web_source", "Web source/config indicator", 70)
  if (/upload|filemanager|ueditor|writefile|move_uploaded_file|createwritestream|files\.write|send_file|download|\.\.\//i.test(lower)) add("file_write", "Upload/file-write/path primitive candidate", 68)
  if (/remote\s*\(\s*["'][^"']+["']\s*,\s*\d{2,5}\s*\)/i.test(text) || /\bnc\s+[^\s]+\s+\d{2,5}\b/i.test(text) || /\b(?:host|server|remote)\s*[:=]\s*[^\s:]+\b/i.test(text) || /\b[A-Za-z0-9.-]+:\d{2,5}\b/.test(text)) add("remote_endpoint", "remote host/port clue adjacent to the challenge bundle", 73)
  ...
  if (BINARY_EXT.test(rel) || sample.subarray(0, 4).equals(Buffer.from([0x7f, 0x45, 0x4c, 0x46])) || sample.subarray(0, 2).toString() === "MZ") add("binary", "binary candidate", 72)
  if (/cipher|encrypt|decrypt|nonce|xor|aes|base64|rot13|vigenere|substitution|modulus/i.test(lower)) add("crypto", "crypto/encoding indicator", 58)
  return out
}
...
function decide(signals: Signal[]) {
  ...
  if (directFlag) return { verdict: "direct_flag", confidence: "high", nextTool: "none", nextTarget: directFlag.rel, spawnSubagent: "no", directSolve: "yes", reason: `verify flag candidate: ${directFlag.detail}` }
  ...
  if (archive && remoteEndpoint && (binary || pwnArchiveName)) {
    return { verdict: "pwn_bundle", confidence: "high", nextTool: "ctf-safe-extract", nextTarget: archive.rel, spawnSubagent: "no", directSolve: "no", reason: `packaged PWN bundle: ${archive.detail}; ${remoteEndpoint.detail}` }
  }
  if (archive) return { verdict: "archive", confidence: "high", nextTool: "ctf-safe-extract", nextTarget: archive.rel, spawnSubagent: "no", directSolve: "no", reason: archive.detail }
  const pcap = pick("pcap")
  if (pcap) return { verdict: "pcap", confidence: "high", nextTool: "ctf-pcap-probe", nextTarget: pcap.rel, spawnSubagent: "no", directSolve: "no", reason: pcap.detail }
  const rsa = pick("rsa")
  if (rsa) return { verdict: "rsa", confidence: "high", nextTool: "ctf-rsa-probe", nextTarget: rsa.rel, spawnSubagent: "no", directSolve: "no", reason: rsa.detail }
  const sourceMap = pick("source_map")
  if (sourceMap) return { verdict: "web_source_map", confidence: "high", nextTool: "ctf-web-source-map", nextTarget: sourceMap.rel, spawnSubagent: "no", directSolve: "no", reason: sourceMap.detail }
  const javaWeb = pick("java_web")
  if (javaWeb) return { verdict: "java_web", confidence: "high", nextTool: "ctf-java-map", nextTarget: ".", spawnSubagent: "no", directSolve: "no", reason: javaWeb.detail }
  const api = pick("api_map")
  if (api) return { verdict: "api_map", confidence: "medium", nextTool: "ctf-api-map", nextTarget: api.rel, spawnSubagent: "no", directSolve: "no", reason: api.detail }
  const web = pick("web_source")
  if (web) return { verdict: "web_source", confidence: "medium", nextTool: "ctf-web-source-map", nextTarget: ".", spawnSubagent: "no", directSolve: "no", reason: web.detail }
  if (binary) return { verdict: "binary", confidence: "medium", nextTool: "ctf-binary-probe", nextTarget: binary.rel, spawnSubagent: "no", directSolve: "no", reason: binary.detail }
  const stego = pick("stego_media")
  if (stego) return { verdict: "stego_media", confidence: "medium", nextTool: "ctf-stego-probe", nextTarget: stego.rel, spawnSubagent: "no", directSolve: "no", reason: stego.detail }
  const crypto = pick("crypto")
  if (crypto) return { verdict: "crypto_or_encoding", confidence: "low", nextTool: "none", nextTarget: crypto.rel, spawnSubagent: "maybe", directSolve: "maybe", reason: crypto.detail }
  return { verdict: "unknown", confidence: "low", nextTool: "none", nextTarget: ".", spawnSubagent: "maybe", directSolve: "no", reason: "no high-confidence fast-lane signal" }
}

export default tool({
  description: "CTF one-shot triage: fastest primary-agent router. Scans high-value files once, emits verdict/confidence/next_tool/next_target/spawn_subagent/direct_solve so the agent can immediately run the best probe without extra reasoning loops.",
  args: {
  ...
  async execute(args, context) {
    ...
    if (decision.verdict === "direct_flag") recommended.push("read only the source line/file for verification, then write the exact flag to agent_flag.txt")
    if (decision.verdict === "unknown") recommended.push("fall back to ctf-quick-triage or inspect only top_files; spawn one subagent only after a category is evidenced")
    if (decision.verdict === "web_source") recommended.push("use source map output to inspect only top routes/config/sinks before any fuzzing")
    if (decision.verdict === "archive") recommended.push("after extraction, rerun ctf-one-shot-triage on the extracted directory")
    if (decision.verdict === "pwn_bundle") recommended.push("after extraction, stay in the PWN fast lane: run ctf-pwn-fast-bootstrap or ctf-binary-probe on the extracted ELF before archive/forensics branching")
