...
async function chooseRunner(lang: string) {
  ...
  if (lang === "powershell") return { cmd: "powershell", args: ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File"], ext: ".ps1" }
  throw new Error(`unsupported lang: ${lang}`)
}

async function startWindowsBackgroundProcess(cmd: string, args: string[], workdir: string, stdoutPath: string, stderrPath: string, launcherPath: string) {
  const psBody = [
...
export default tool({
  description: "CTF background job: start, inspect, stop, and read detached local Python/Node/PowerShell jobs for long-running brute force or analysis tasks.",
  args: {
  ...
  async execute(args, context) {
    ...
    if (op === "start") {
      ...
      const scriptPath = args.scriptPath ? resolveInsideWorkspace(context.directory, args.scriptPath) : path.join(jobDir, `run${runner.ext}`)
      ...
      const meta = { id, lang: args.lang || "python", pid, scriptPath, workdir, stdoutPath, stderrPath, startedAt: new Date().toISOString(), argv: args.argv || [] }
      await writeFile(metaPath, JSON.stringify(meta, null, 2), "utf8")
      const payload = { ok: true, ...meta }
      if (args.jsonOnly) return JSON.stringify(payload, null, 2)
      return ["CTF_BACKGROUND_JOB:", "- operation: start", "- ok: true", `- id: ${id}`, `- pid: ${pid}`, `- stdout: ${stdoutPath}`, `- stderr: ${stderrPath}`].join("\n")
    }
    ...
    if (op === "status") {
      ...
      const payload = { ok: true, id, running: isPidAlive(meta.pid), pid: meta.pid, startedAt: meta.startedAt, stdoutTail: stdout.split(/\r?\n/).slice(-20).join("\n"), stderrTail: stderr.split(/\r?\n/).slice(-20).join("\n") }
      if (args.jsonOnly) return JSON.stringify(payload, null, 2)
      return ["CTF_BACKGROUND_JOB:", "- operation: status", `- id: ${id}`, `- running: ${payload.running}`, `- pid: ${payload.pid}`, `- started_at: ${payload.startedAt}`, "- stdout_tail:", payload.stdoutTail, "- stderr_tail:", payload.stderrTail].join("\n")
    }
