...
function extractGotTargets(text: string) {
  ...
  for (const n of names) if (new RegExp(`${n}.*(?:@got|got|plt)|(?:@got|got).*${n}`, "i").test(text)) targets.push(`${n}@got`)
  return uniq(targets)
}

export default tool({
  description: "CTF PWN primitive compressor: convert high-value disassembly/control signals into a prioritized minimal one-variable primitive probe with oracle/falsifier.",
  args: {
    evidence: tool.schema.string().describe("Disassembly/decompilation/notes with callsites, rbp control, GOT/global targets, and offset/control evidence."),
  ...
  async execute(args) {
    ...
    const frameArg = /lea\s+r(ax|di|si|dx|cx|8|9),\s*\[rbp\s*-\s*0x[0-9a-f]+\]/i.test(evidence) || /mov\s+rdi,\s*\[rbp\s*-\s*0x[0-9a-f]+\]/i.test(evidence)
    const savedRbp = /saved rbp|rbp control|overwrite rbp|leave\s*;?\s*ret|\bleave\b|payload.*p64\([^)]*rbp/i.test(lower)
    const printCall = /printf|puts|write/.test(lower)
    const candidates: any[] = []
    if (frameArg && printCall) {
      const callsite = args.callsite || callsites.find((c) => /printf|puts|write/.test(c.callee))?.address || "<callsite_before_arg_setup>"
      ...
      candidates.push({
        ...
        probe: k ? `payload = b'A'*${off} + p64(${target} + 0x${k.toString(16)}) + p64(${callsite})` : `payload = b'A'*${off} + p64(${target} + k) + p64(${callsite})`,
        confirm: "target bytes or raw libc/GOT pointer bytes are printed before crash/EOF",
        falsify: "the original callsite is reached with rbp formula but does not read/print target, or target is unreadable under this callsite",
        do_not_do_yet: ["fake-stack libc stability", "GOT-page rewrite loop", "format-write closure", "one_gadget polishing"],
      })
    }
    if (/win|print_flag|backdoor/.test(lower)) candidates.push({ name: "DIRECT_WIN", priority: "P0", value: "high", probe: "minimal ret2win/control transfer", confirm: "flag/win executes", falsify: "unavailable control or required state impossible" })
    if (/format|%p|%s|%n/.test(lower)) candidates.push({ name: "FMT_LEAK_FIRST", priority: candidates.length ? "P1" : "P0", value: "medium", probe: "read-only positional leak map", confirm: "stable leak offset", falsify: "input is not interpreted as format" })
    const payload = {
    ...
    return [
      ...
      ...(candidates.length ? candidates.map((c) => `- ${c.priority} ${c.name}: formula=${c.formula || ""}; probe=${c.probe}; confirm=${c.confirm}; falsify=${c.falsify}`) : ["- none"]),
      "primitive_lock_card:",
