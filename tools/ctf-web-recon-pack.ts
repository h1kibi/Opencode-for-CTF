...
function routes(text: string) {
  return uniq(Array.from(text.matchAll(/(?:href|src|action)=["']([^"'#]+)|["'`]((?:\/[A-Za-z0-9_.~!$&'()*+,;=:@%-]+){1,8}(?:\?[A-Za-z0-9_.~!$&'()*+,;=:@%/?-]*)?)["'`]/g), (m) => m[1] || m[2]).filter((x) => x?.startsWith("/")), 120)
}

function scripts(text: string, base: URL) {
  return uniq(Array.from(text.matchAll(/src=["']([^"']+\.js[^"']*)/gi), (m) => {
    try {
...
function classify(all: string, headers: string[], routeList: string[]): Finding[] {
  ...
  const add = (label: string, re: RegExp, next: string, score: number) => { if (re.test(text)) findings.push({ label, evidence: re.source, next, score }) }
  add("direct flag-like content", /[A-Za-z0-9_@.-]{2,32}\{[^\r\n}]{1,200}\}/, "verify candidate immediately and stop broad recon", 100)
  add("Java/Spring/Tomcat surface", /spring|jsessionid|actuator|tomcat|jetty|shiro|struts|whitelabel/i, "run ctf-java-analyze-pack or Java control-plane checks", 85)
  add("Swagger/OpenAPI route oracle", /swagger|openapi|api-docs|redoc/i, "fetch OpenAPI/Swagger variants before route fuzzing", 80)
  add("debug/source/config leak", /debug|traceback|stack trace|sourceMappingURL|\.git|\.env|actuator/i, "read source/config candidate with one safe request", 78)
  add("auth/session workflow", /login|logout|register|reset|csrf|xsrf|session|jwt|oauth|saml/i, "run state/authz map when accounts or IDs exist", 68)
  add("upload/file surface", /upload|multipart|download|file|export|import|preview|render/i, "run focused file/upload canary or differential", 64)
  add("GraphQL/API surface", /graphql|\/api\b|fetch\(|axios\./i, "map API corpus and run one differential probe", 62)
  add("browser/admin-bot/runtime surface", /bot|report|admin.?bot|puppeteer|selenium|postMessage|innerHTML/i, "run runtime map; self-test marker before privileged trigger", 60)
  return findings.sort((a, b) => b.score - a.score).slice(0, 8)
}

export default tool({
  description: "CTF Web recon macro pack: one low-volume URL fetch plus static route/runtime classification and focused next-tool routing.",
  args: {
  ...
  async execute(args) {
    ...
    const nextTool = findings[0]?.label.includes("Java") ? "ctf-java-analyze-pack" : findings.length ? "ctf-decision-state" : "ctf-web-blackbox-map"
    return [
      "# CTF Web Recon Pack",
      `url: ${base.toString()}`,
      `mode: ${mode}`,
      ...
      `next_tool: ${nextTool}`,
      "findings:",
      ...(findings.length ? findings.map((f, i) => `- #${i + 1} score=${f.score} ${f.label}; next=${f.next}`) : ["- none"]),
      "routes:",
