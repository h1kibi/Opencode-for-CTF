...
function lineHints(lines: string[]) {
  ...
  for (const raw of lines) {
    ...
    if (/puts\s*\(|printf\s*\(|write\s*\(/.test(line) && /choice|menu|name|size|index|length/i.test(line))
      contracts.push("menu_prompt_output")
  }
...
function recommendActions(contracts: string[]) {
  ...
  if (contracts.some((x) => x.startsWith("read_exact:"))) {
    ...
    out.push(
      `Prefer send()/sendafter() with exact byte count ${exact}; avoid implicit newline unless the parser explicitly strips it.`,
    )
    out.push(
      "After an exact-length read, immediately model whether a newline remains in the menu buffer before the next prompt-driven step.",
    )
  }
  if (contracts.includes("fgets_line_includes_newline_if_room") || contracts.includes("gets_line_mode")) {
    out.push(
      "Prefer sendline()/sendlineafter() for line-based reads; account for newline retention or trimming in subsequent comparisons.",
    )
  }
  if (contracts.includes("scanf_int_whitespace_consumption") || contracts.includes("numeric_parse_expect_text_line")) {
    out.push(
      "Use text helpers for numeric menu selections and separate them from raw payload stages to avoid leftover bytes contaminating later reads.",
    )
  }
  if (contracts.includes("cpp_operator_shr_token_read") && contracts.includes("getline_line_read_after_token_risk")) {
    out.push(
      "Detected operator>> plus getline mix; expect leftover newline state. Prefer sendline() for numeric selections, then sendafter(content prompt, payload + '\\n') for getline-backed content stages.",
    )
    out.push(
      "For menu helpers, keep numeric choice/index/len on sendline(), but content reads on explicit sendafter()/sendlineafter() matched to the content prompt.",
    )
  }
  if (contracts.includes("recv_exact_or_socket_read")) {
    out.push(
      "Assume the program may wait for an exact count; verify short-send behavior before blaming the exploit route.",
    )
  }
  if (!out.length)
    out.push(
      "No strong read/write contract found; keep one helper per menu phase and log prompt/output boundaries before payload mutation.",
    )
  return out
}
...
export default tool({
  description:
    "CTF pwn menu contract probe: inspect source snippets or notes for exact-length vs line-oriented reads and recommend send/sendline helper contracts before menu bytes pollute later probes.",
  args: {
    evidence: tool.schema.string().describe("Source snippet, decompilation lines, notes, or menu I/O observations."),
    jsonOnly: tool.schema.boolean().optional().describe("Return JSON only. Default false."),
  },
  async execute(args) {
    const lines = String(args.evidence || "")
      .split(/\r?\n/)
      .slice(0, 400)
    ...
    const payload = {
      ...
      helper_contract: contracts.some((x) => x.startsWith("read_exact:"))
        ...
        : contracts.includes("fgets_line_includes_newline_if_room") || contracts.includes("gets_line_mode")
          ? contracts.includes("cpp_operator_shr_token_read") &&
            contracts.includes("getline_line_read_after_token_risk")
