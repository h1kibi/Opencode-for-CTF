...
export default tool({
  description:
    "CTF pwn playbook router: convert ctf-binary-probe output or pwn evidence into top-3 exploit strategy seeds with confirm/falsify probes.",
  args: {
    ...
    mode: tool.schema
      ...
      .optional()
      .describe(
        "direct | medium | hard. direct returns one fastest route, hard returns deeper top routes. Default medium.",
      ),
    maxRoutes: tool.schema.number().optional().describe("Maximum routes to return. Default 3, hard cap 5."),
    jsonOnly: tool.schema
      .boolean()
      .optional()
      .describe("Return machine-readable JSON instead of Markdown-like text. Default false."),
  },
  async execute(args) {
    const raw = args.evidence || ""
    const text = raw.toLowerCase()
    if (text.trim().length < 20)
      return "BLOCK: evidence is too short; run ctf-binary-probe or provide checksec/protocol/primitive clues"

    const mode = ["direct", "medium", "hard"].includes((args.mode || "").toLowerCase())
      ? (args.mode || "medium").toLowerCase()
      : "medium"
    const routes: Route[] = []
    ...
    const orwPreferred = has(
      text,
      /prefer_direct_file_read_or_orw_route_over_shell|orw_route_candidate|open-read-write|direct file-read|blocked shell/,
    )
    const syscallAbiKnown = has(
      text,
      /syscall_abi_hints|arch_hint:\s*(amd64|i386|arm64)|syscall_number:\s*(rax|eax|x8)/,
    )
    const ret2csuComplete = has(
      text,
      /gadget_shape:\s*complete_pop_and_call_shape|ret2csu_candidate_worth_testing_after_control_proof/,
    )
    const ret2csuPartial = has(
      text,
      /ret2csu_partial_shape|gadget_shape:\s*(pop_shape_only|call_shape_only|symbol_only)/,
    )
    ...
    const modernGlibc = has(
      text,
      /safe_linking_likely|malloc_free_hooks_removed_or_unreliable|do_not_default_to_malloc_free_hook_targets|glibc_version:\s*2\.(3[4-9]|4\d)/,
    )
    const formatMapped = has(
      text,
      /pwn_format_map:|positional_behavior_clue|leak_class_candidates|write_gates|write_capability_candidate/,
    )
    const formatWriteCandidate = has(
      text,
      /write_capability_candidate|positional_write_possible_if_offset_stable|n_specifier_seen_but_writability_unproven/,
    )
    ...
    const writableLongLived = has(
      text,
      /\.bss|global data|global buffer|writable global|heap state|long-lived memory|parser buffer|scanf|fgets|read\(|recv\(|strcpy|memcpy/,
    )
    ...
    const localWorksRemoteFails =
      has(text, /local_success_signal:\s*true/) && has(text, /remote_failure_signal:\s*true/)
    ...
    const scriptRanOkOnly =
      has(text, /script_ran_ok:\s*true|"script_ran_ok"\s*:\s*true/) && !flagDetected && !shellDetected
    const frameIndexedCallsite = has(
      text,
      /frame[-_ ]indexed|rbp[-_ ]relative|\[rbp-0x[0-9a-f]+\].*(printf|puts)|lea\s+r(ax|di),\s*\[rbp-0x[0-9a-f]+\].*call.*(printf|puts)/s,
    )

    if (frameIndexedCallsite)
      add(
        routes,
        {
          ...
          firstProbe:
            "Compress original callsite into one probe: saved_rbp=got/global_target+k, saved_rip=callsite_before_arg_setup; leak GOT/global bytes before fake-stack or broad ROP.",
          confirm: "Target bytes or raw libc/GOT pointer bytes are printed before crash/EOF.",
          falsify:
            "The original callsite is reached with the rbp formula but does not consume/print target, or target is unreadable under that callsite.",
          stopRule:
            "Do not debug fake-stack libc crashes, GOT-page rewrite loops, or one_gadget constraints before this primitive is tested.",
          nextPatternQuery: "frame indexed callsite leak saved rbp printf got rbp target plus offset",
        },
        120 + (mode === "direct" ? 10 : 0),
        ["frame-indexed callsite", "saved rbp argument selector"],
      )
    ...
    if (win && pie === false)
      add(
        routes,
        {
          ...
          firstProbe:
            "Prove offset/control with cyclic or debugger, then call win/print_flag with required alignment/arguments.",
        ...
        95 + (mode === "direct" ? 15 : 0),
        ["win-like symbol/string", "PIE disabled"],
      )

    if (canary === false && nx === true && (pie === false || systemBinsh || dangerousRead || ipControlled))
      add(
        routes,
        {
          id: "ret2libc-or-rop",
          ...
          firstProbe:
            "Prove RIP control; if PIE/ASLR/libc unknown, leak puts/write/GOT or stack/libc pointer before stage2.",
        ...
        85 +
          ...
          (ipControlled ? 8 : 0) +
          (offsetKnown ? 3 : 0) -
          (oneGadgetConstrained ? 3 : 0) +
          (mode === "direct" && pie === false ? 6 : 0),
        [
          "NX enabled",
          "canary absent",
          pie === false ? "PIE disabled" : "leak may be needed",
          libcResolved ? "libc resolver evidence" : "libc unresolved",
          ipControlled ? "crash probe shows IP control" : "control proof needed",
          oneGadgetConstrained ? "one_gadget constraints need proof" : "classic ROP preferred before one_gadget",
        ],
      )

    if (nx === false && canary === false)
      add(
        routes,
        {
          id: "shellcode",
          ...
          firstProbe:
            "Confirm executable stack/region, bad chars, input transform, register state, and seccomp before shellcode.",
    ...
    if (fmt || formatMapped)
      add(
        routes,
        {
          ...
          firstProbe:
            "Use ctf-pwn-format-map output or build a read-only positional offset map; classify leaks before any %n write.",
        ...
        88 +
          (formatMapped ? 8 : 0) +
          (formatWriteCandidate ? 4 : 0) +
          (relroPartial ? 5 : 0) +
          (mode === "hard" ? 4 : 0),
        [
          formatMapped ? "format-map evidence" : "printf/format clue",
          formatWriteCandidate ? "%n/write candidate gated" : "read-first map required",
          relroPartial ? "GOT write may be possible" : relroFull ? "Full RELRO" : "RELRO unknown",
        ],
      )
    ...
    if (heap || heapMapped)
      add(
        routes,
        {
          ...
          firstProbe:
            "Use ctf-pwn-heap-menu-map output: reduce add/delete/edit/show lifecycle, then prove leak/UAF/reuse/write before named techniques.",
        ...
        82 + (heapMapped ? 8 : 0) + (heapLeakSurface ? 4 : 0) + (modernGlibc ? 3 : 0) + (mode === "hard" ? 8 : 0),
        [
          heapMapped ? "heap-menu-map evidence" : "heap allocator clue",
          heapLeakSurface ? "leak surface possible" : "leak surface unknown",
          modernGlibc ? "modern glibc constraints" : "allocator version still needed",
        ],
      )
    ...
    if ((writableLongLived && outputPath) || adjacency)
      add(
        routes,
        {
          ...
          firstProbe:
            "Audit adjacent long-lived objects and test one path/length/state/string overwrite that reuses an existing puts/printf/write/send consumer.",
          confirm:
            "One adjacency probe changes secret-bearing output, path consumption, length/state behavior, or extends an existing output path.",
          falsify:
            "Writable long-lived memory exists but nearby consumers or output paths cannot be influenced behaviorally.",
          stopRule:
            "Do not jump to shell/ROP/file-write expansion before one adjacency/output-hijack probe is tried when secret-bearing output is already nearby.",
          nextPatternQuery:
            "pwn data-only corruption bss global adjacent string output hijack path overwrite length state",
        },
        91 +
          (outputPath ? 8 : 0) +
          (adjacency ? 8 : 0) +
          (has(text, /partial flag|prefix of flag|almost prints/) ? 6 : 0) +
          (mode === "hard" ? 4 : 0),
        [
          writableLongLived ? "writable long-lived memory" : "adjacency clue",
          outputPath ? "existing output path present" : "output path inferred",
          adjacency ? "adjacent overwrite clue" : "adjacency should be audited",
        ],
      )

    if (seccomp || orwPreferred)
      add(
        routes,
        {
          ...
          firstProbe:
            "Run or use ctf-pwn-syscall-orw-check output: verify ABI registers, syscall allowlist, writable buffer, and stdout/socket write closure before shell attempts.",
        ...
        90 + (orwPreferred ? 12 : 0) + (syscallAbiKnown ? 4 : 0) + (mode === "hard" ? 6 : 0),
        [
          seccomp ? "seccomp/sandbox clue" : "ORW/direct file-read clue",
          syscallAbiKnown ? "syscall ABI known" : "syscall ABI needs check",
        ],
      )
    ...
    if (ret2csuComplete || ret2csuPartial || has(text, /__libc_csu_init|ret2csu/))
      add(
        routes,
        {
          ...
          firstProbe:
            "Use ctf-pwn-ret2csu-check output: require control proof, paired pop/call csu gadget shape, callable pointer, rbx/rbp loop setup, and argument mapping.",
          confirm: "A controlled csu call reaches a benign imported function with expected arguments under debugger.",
          falsify:
            "Missing paired gadget shape, no callable pointer, no control proof, or simpler pop-gadget route dominates.",
          stopRule:
            "Do not promote ret2csu if only symbol/partial shape exists and no missing-gadget reason is proven.",
          nextPatternQuery: "ret2csu pop rbx rbp r12 r13 r14 r15 call gadget missing pop rdx",
        },
        72 + (ret2csuComplete ? 16 : 0) - (ret2csuPartial ? 10 : 0) + (pie === false ? 4 : 0),
        [
          ret2csuComplete ? "complete ret2csu gadget shape" : ret2csuPartial ? "partial ret2csu shape" : "ret2csu clue",
          pie === false ? "PIE disabled" : "PIE/code base may be needed",
        ],
      )
    ...
    if (cet)
      add(
        routes,
        {
          ...
          firstProbe:
            "If indirect branch crashes, distinguish wrong address from CET/IBT/valid-target constraint and consider syscall-oriented routes.",
    ...
    if (crashProbe && !ipControlled)
      add(
        routes,
        {
          ...
          firstProbe:
            "Use ctf-pwn-crash-probe details to adjust protocol, input length, newline/null handling, argv/stdin path, and crash site before exploit route selection.",
        ...
        70 + (crashNoControl ? 8 : 0),
        ["crash probe evidence", crashNoControl ? "crash without IP control" : "control unresolved"],
      )
    ...
    if (remoteDrift)
      add(
        routes,
        {
          ...
          firstProbe:
            "Use ctf-pwn-remote-drift-check ranked_rechecks: test exactly one drift hypothesis such as libc/ld, prompt sync, alignment, seccomp, timeout/EOF, or fork behavior.",
        ...
        86 + (localWorksRemoteFails ? 10 : 0) + (driftLibc ? 5 : 0) + (driftPrompt ? 4 : 0),
        [
          localWorksRemoteFails ? "local proof remote failure" : "remote drift evidence",
          driftLibc
            ? "libc/ld drift clue"
            : driftPrompt
              ? "prompt sync drift clue"
              : "drift class needs one-variable test",
        ],
      )

    if (runnerSummary && !flagDetected)
      add(
        routes,
        {
          ...
          firstProbe:
            "Use ctf-pwn-runner summary: distinguish flag, shell, crash, timeout, and script_ran_ok; convert shell to flag read or debug crash/timeout before final reporting.",
          ...
          stopRule:
            "Do not report success from script_ran_ok alone; SIGSEGV after payload is failure unless flag was already captured.",
          nextPatternQuery: "pwn runner script ran ok no flag shell crash timeout final verification",
        },
        92 + (shellDetected ? 8 : 0) - (runnerCrash ? 8 : 0) - (runnerTimeout ? 6 : 0) - (scriptRanOkOnly ? 4 : 0),
        [
          shellDetected
            ? "shell detected but flag not verified"
            : scriptRanOkOnly
              ? "script ran ok is not success"
              : "runner needs closure",
          runnerCrash ? "runner crash" : runnerTimeout ? "runner timeout" : "no crash/timeout clue",
        ],
      )
    ...
    if (!routes.length)
      add(
        routes,
        {
          ...
          firstProbe:
            "Model protocol and reproduce crash/oracle; use cyclic/debugger/source audit to classify control/leak/write/logic primitive.",
    ...
    return [
      ...
      "decision_contract:",
      mode === "direct"
        ? "- Direct mode: run only the top route's first_probe unless new evidence contradicts it."
        : "- Pick at most one route for the next probe, or top-3 for ctf-decision-state ranking.",
      mode === "hard"
        ? "- Hard mode: preserve alternative routes as pivots, but still run one variable per probe."
        : "- Escalate to hard mode only when two strong routes fail or the target is clearly multi-stage.",
