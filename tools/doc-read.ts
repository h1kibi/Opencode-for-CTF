...
function choosePython() {
  if (process.platform === "win32") return process.env.PYTHON || "python"
...
export default tool({
  ...
  async execute(args: Args, context: any) {
    ...
    const candidateRoots = Array.from(new Set([context.worktree, context.directory].filter(Boolean) as string[]))
    const candidateScripts = candidateRoots.flatMap((root) => [
      path.join(root, ".opencode/tools/document_extract.py"),
      path.join(root, "tools/document_extract.py"),
    ])
    const script = candidateScripts.find((item) => existsSync(item))
    if (!script) {
      throw new Error(`Missing helper script. Tried: ${candidateScripts.join(", ")}`)
    }
    ...
    return new Promise<string>((resolve, reject) => {
      const proc = spawn(choosePython(), [script], {
