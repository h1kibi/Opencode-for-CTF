...
function linesBySection(text: string) {
  ...
  for (const raw of String(text || "").split(/\r?\n/)) {
    ...
    if (heading) {
      current = heading[1].replace(/^#+\s*/, "").replace(/:$/, "").trim().toLowerCase()
...
function scoreOverlap(a: string, b: string) {
  ...
  let score = 0
  for (const token of ["heap", "uaf", "overlap", "fd", "bk", "size", "seccomp", "orw", "jmp rsp", "shellcode", "ret2libc", "flag", "closure", "glibc", "tcache"]) {
...
export default tool({
  description: "CTF PWN WP diff: compare a pasted WP excerpt with the current route notes and emit a compact structured diff of route, primitive, blocker, and shortest-closure differences.",
  args: {
  ...
  async execute(args) {
    ...
    const recommendations = [
      wpOnly.length ? "Promote only the smallest WP-only step that shortens the current closure path; do not import the whole chain blindly." : "Current route already overlaps strongly with the WP; focus on implementation details or runtime drift.",
      curOnly.length ? "Check whether current extra branches are genuinely shorter than the WP closure path or just harder side routes." : "No large current-only branch detected; compare runtime assumptions next.",
      "Extract one diff as a next probe: route, primitive, blocker, or closure owner — not all four at once.",
    ]
    const payload = { schema_version: "pwn_wp_diff.v1", shared, wp_only: wpOnly, current_only: curOnly, recommendations }
