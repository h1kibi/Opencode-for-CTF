...
export default tool({
  description:
    "CTF pwn stack frame solver: input function disassembly and emit precise frame offsets, rbp-derived expressions, and candidate leak surfaces.",
  args: {
  ...
  async execute(args, context) {
    ...
    if (
      /lea\s+r(ax|di|si|dx|cx|8|9),\s*\[rbp-0x[0-9a-f]+\]/.test(lower) &&
      /mov\s+rdi,\s*r(ax|di|si|dx|cx|8|9)/.test(lower) &&
      /call.*<(printf|puts)@plt>|call.*printf|call.*puts/.test(lower)
    ) {
      callsiteReuse.push("frame_indexed_first_arg_control")
    }
    if (/leave\s*;?\s*ret|\bleave\b/.test(lower) && /rbp/.test(lower))
      callsiteReuse.push("leave_ret_pseudostack_candidate")
    if (/call.*<(printf|puts)@plt>|call.*printf|call.*puts/.test(lower) && /rbp-0x[0-9a-f]+/.test(lower))
      callsiteReuse.push("original_callsite_reuse_candidate")
    const midFunctionRisks = [] as string[]
    if (callsiteReuse.includes("original_callsite_reuse_candidate"))
      midFunctionRisks.push("mid_function_reentry_may_skip_prologue_initialization")
    if (/printf/.test(lower)) midFunctionRisks.push("printf_varargs_state_may_be_unstable_on_mid_function_reentry")
    if (/leave\s*;?\s*ret|\bleave\b/.test(lower))
      midFunctionRisks.push("verify_saved_rbp_saved_ret_and_rsp_migration_before_closure")
    ...
    const callsiteMatch = text.match(
      /\b([0-9a-f]{6,16})[:\s].*call\s+.*(?:<(printf|puts|write)[^>]*>|printf|puts|write)/i,
    )
    const callsite = callsiteMatch ? `0x${callsiteMatch[1]}` : "<callsite_before_arg_setup>"
    const highPriorityPrimitive =
      callsiteReuse.includes("frame_indexed_first_arg_control") ||
      callsiteReuse.includes("original_callsite_reuse_candidate")
        ? {
            ...
            minimal_probe: frameArgOffset
              ? `payload = b'A'*<offset_to_saved_rbp> + p64(target + ${frameArgOffset}) + p64(${callsite})`
              : `payload = b'A'*<offset_to_saved_rbp> + p64(target + k) + p64(${callsite})`,
            oracle: "target bytes or raw GOT/libc pointer bytes printed before crash/EOF",
            blockers_before_closure: [
              "do not debug fake-stack libc calls until this probe is tried",
              "complex closure failure does not falsify original callsite primitive",
            ],
          }
        : null
    const payload = {
    ...
    return [
      ...
      ...(payload.candidate_leak_surfaces.length
        ? payload.candidate_leak_surfaces.map((x: string) => `- ${x}`)
        : ["- none"]),
      "callsite_reuse_signals:",
      ...(payload.callsite_reuse_signals.length
        ? payload.callsite_reuse_signals.map((x: string) => `- ${x}`)
        : ["- none"]),
      ...
      ...(payload.mid_function_reentry_risks.length
        ? payload.mid_function_reentry_risks.map((x: string) => `- ${x}`)
        : ["- none"]),
      "red_flag_tags:",
