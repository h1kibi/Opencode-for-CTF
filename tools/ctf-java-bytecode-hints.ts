...
const interestingClass = /(Controller|Servlet|Filter|Interceptor|Security|Config|Shiro|Login|Admin|User|File|Upload|Download|Template|View|Xml|Url|Proxy|Http|Sql|Mapper|Service|Repository|Actuator|H2)/i
const interestingString = /(RequestMapping|GetMapping|PostMapping|PutMapping|DeleteMapping|PathVariable|RequestParam|RequestBody|JSESSIONID|rememberMe|actuator|h2-console|shiro|login|admin|flag|upload|download|\.jsp|\.ftl|\.vm|\.html|jdbc:|select |insert |update |delete |\$\{|#\{|http:|https:|file:|DocumentBuilder|RestTemplate|ObjectInputStream|Fastjson|parseObject)/i

async function safeJavap(classFile: string, cwd: string) {
  try {
    const { stdout, stderr } = await execFile("javap", ["-v", "-p", classFile], { cwd, timeout: 6000, maxBuffer: 1024 * 1024 })
    return `${stdout}${stderr ? `\n${stderr}` : ""}`
  } catch (err) {
    return `<javap failed: ${err}>`
  }
}
...
export default tool({
  description: "CTF Java bytecode hints: scan extracted .class files with names and javap constant-pool snippets to recover route annotations, strings, class roles, sinks, and decompile targets for bytecode-only Java Web challenges.",
  args: {
    target: tool.schema.string().describe("Extracted Java archive/source directory containing .class files"),
    maxClasses: tool.schema.number().optional().describe("Maximum interesting classes to javap. Default 24, hard cap 80."),
  },
  async execute(args, context) {
    ...
    if (rootStat.isFile()) {
      if (!root.endsWith(".class")) throw new Error("target file must be a .class file, or use an extracted class directory")
    ...
    ].filter((v, i, a) => a.indexOf(v) === i).slice(0, maxClasses)
    ...
    for (const file of javapTargets) {
      ...
      const ann = pick(lines, /(RuntimeVisibleAnnotations|RequestMapping|GetMapping|PostMapping|WebServlet|Controller|RestController|RequestParam|PathVariable|RequestBody)/i, 50)
      if (hits.length || ann.length) {
        snippets.push(`class: ${rel}\n${[...ann, ...hits].slice(0, 60).map((x) => `  - ${x.trim()}`).join("\n")}`)
      ...
      if (/FileInputStream|FileOutputStream|Files\.|Paths\.|MultipartFile|ZipInputStream/i.test(out)) sinks.add(`file/upload: ${rel}`)
      if (/Statement|PreparedStatement|JdbcTemplate|createNativeQuery|select |insert |update |delete |\$\{/i.test(out)) sinks.add(`sql: ${rel}`)
      if (/TemplateEngine|Freemarker|FreeMarker|Velocity|ExpressionParser|SpelExpressionParser|Ognl/i.test(out)) sinks.add(`template/expression: ${rel}`)
      if (/ObjectInputStream|readObject|parseObject|ObjectMapper|Yaml|XStream|XMLDecoder/i.test(out)) sinks.add(`deserialization/parser: ${rel}`)
      if (/DocumentBuilder|SAXParser|XMLInputFactory|SAXReader|TransformerFactory/i.test(out)) sinks.add(`xxe: ${rel}`)
      if (/RestTemplate|HttpClient|WebClient|openConnection|OkHttpClient|URL\b|URI\b/i.test(out)) sinks.add(`ssrf/url-fetch: ${rel}`)
    }

    const queue: string[] = []
    if (routeHints.size) queue.push("Route annotations found in bytecode: decompile the listed controller/servlet classes first and build route/input table")
    if (sinks.size) queue.push("Sink hints found in bytecode: decompile only sink classes and trace backward to routes")
    if (roleClasses.length) queue.push("Role-like class names found: prioritize Controller/Servlet/Filter/Security/Config/Admin/File/Upload classes")
    if (!queue.length) queue.push("No strong bytecode hints; combine config/dependency maps and decompile smallest package containing app classes")
