...
function resolveInsideWorkspace(contextDir: string, input: string) {
  ...
  if (rel.startsWith("..") || path.isAbsolute(rel))
    throw new Error(`target must stay inside current workspace: ${input}`)
  return target
}
...
export default tool({
  description:
    "CTF APK resource direct reader: list custom resources and return matching value lines for a resource id or name using aapt/apkanalyzer output.",
  args: {
    apk: tool.schema.string().describe("Workspace-relative APK path"),
    query: tool.schema
      .string()
      .optional()
      .describe("Resource id like 0x7f0a0001 or resource name like app_name, main_title, layout/main."),
  ...
  async execute(args, context) {
    ...
    const aapt = await safeExec("aapt", ["dump", "--values", "resources", apk], cwd, 20000)
    const apkanalyzer = await safeExec("apkanalyzer", ["resources", "print", apk], cwd, 20000)
