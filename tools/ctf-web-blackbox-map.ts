...
function extractForms(text: string, base: URL) {
  return Array.from(text.matchAll(/<form\b([\s\S]*?)>([\s\S]*?)<\/form>/gi), (m) => {
    ...
    const inputs = unique(
      Array.from(body.matchAll(/(?:name|id)=["']([^"']+)/gi), (x) => x[1]),
      30,
    )
    const buttons = unique(
      Array.from(body.matchAll(/<button\b[\s\S]*?(?:name|id)=["']([^"']+)/gi), (x) => x[1]),
      10,
    )
...
function extractLinks(text: string, base: URL) {
  const hrefs = Array.from(
    text.matchAll(/(?:href|src|action)=["']([^"'#]+)/gi),
    (m) => sameOrigin(base, m[1])?.pathname,
  ).filter((x): x is string => Boolean(x))
  const rawRoutes = Array.from(
    text.matchAll(/["'`]((?:\/[A-Za-z0-9_.~!$&'()*+,;=:@%-]+){1,10}(?:\?[A-Za-z0-9_.~!$&'()*+,;=:@%/?-]*)?)["'`]/g),
    (m) => m[1],
  )
  return unique(
    [...hrefs, ...rawRoutes].filter((x) => x.startsWith("/")),
    160,
  )
}
...
function extractScripts(text: string, base: URL) {
  return unique(
    Array.from(text.matchAll(/src=["']([^"']+\.js[^"']*)/gi), (m) => sameOrigin(base, m[1])?.toString()).filter(
...
function routeHints(text: string) {
  const hits = Array.from(
    text.matchAll(
      /\/(?:api|admin|debug|dev|flag|graphql|internal|upload|file|download|preview|render|bot|report|login|logout|register|reset|profile|user|users|search|export|import|webhook|callback|oauth|saml|actuator)[A-Za-z0-9_./?=&%:-]*/gi,
    ),
    (m) => m[0],
  )
  return unique(
    hits.map((x) => x.slice(0, 180)),
    140,
  )
}
...
function extractFetchTargets(text: string) {
  const hits = [
    ...
    ...Array.from(
      text.matchAll(/\bXMLHttpRequest\b|\.open\(\s*["'`](GET|POST|PUT|PATCH|DELETE)["'`]\s*,\s*["'`]([^"'`]+)/g),
      (m) => m[2] ?? m[0],
    ),
  ]
  return unique(
...
function extractStorageKeys(text: string) {
  const hits = Array.from(
    text.matchAll(/\b(localStorage|sessionStorage)\s*\.\s*(?:getItem|setItem|removeItem)\s*\(\s*["'`]([^"'`]+)["'`]/g),
    (m) => `${m[1]}:${m[2]}`,
  )
  const direct = Array.from(
    text.matchAll(/\b(localStorage|sessionStorage)\s*\.\s*([A-Za-z0-9_$-]{2,80})/g),
    (m) => `${m[1]}:${m[2]}`,
  )
...
function extractServiceWorkers(text: string) {
  return unique(
    Array.from(text.matchAll(/serviceWorker\.register\s*\(\s*["'`]([^"'`]+)/g), (m) => m[1]),
    20,
  )
}
...
function findTech(headersText: string, text: string) {
  ...
  return {
    ...
    frontendHints: unique(frontendHints),
    authHints: unique(authHints),
    proxyCacheHints: unique(proxyCacheHints),
  }
}
...
function classifySurface(
  ...
  if (/graphql/i.test(combined) || routes.some((x) => /graphql/i.test(x)))
    add("GraphQL endpoint", "graphql schema/error/GET-CSRF calibration | V=4 C=2 IG=4 R=1 SD=0 S=4")
  if (/swagger|openapi|api-doc|redoc/i.test(combined))
    add("API documentation leak", "OpenAPI/Swagger route expansion before fuzzing | V=4 C=1 IG=4 R=1 SD=0 S=5")
  if (/upload|multipart|file_input=yes/i.test(combined))
    add("upload surface", "upload validation/storage harmless canary | V=4 C=2 IG=3 R=3 SD=2 S=3")
  if (/admin.?bot|report|bot|xss|phantomjs|selenium|puppeteer|headless|share|preview/i.test(combined)) {
    add(
      "admin bot / browser runtime surface",
      "one harmless bot/runtime fingerprint before payload | V=5 C=3 IG=4 R=4 SD=2 S=2",
    )
    browserNeeds.push("admin bot/runtime differences")
  }
  if (/debug|traceback|stack trace|werkzeug|express error|django|flask debug|__debugger__|actuator/i.test(combined))
    add("debug/source/config leak surface", "debug/source/config disclosure fetch | V=4 C=1 IG=4 R=1 SD=0 S=5")
  if (/login|register|session|csrf|xsrf|jwt|eyJ[A-Za-z0-9_-]+\.|oauth|saml/i.test(combined))
    add("auth/session/token surface", "auth/session/CSRF differential map | V=4 C=2 IG=4 R=1 SD=0 S=4")
  if (/redirect_uri|redirect=|return=|next=|url=|callback=|webhook/i.test(combined))
    add(
      "URL parser / redirect / SSRF-adjacent surface",
      "safe URL parser differential with allowlisted benign target | V=4 C=2 IG=4 R=2 SD=0 S=3",
    )
  if (/search|query|sqlite|mysql|postgres|sql|mongo|where\[|\$ne|\$regex/i.test(combined))
    add("query/parser injection surface", "one-variable parser/oracle calibration | V=3 C=2 IG=3 R=1 SD=0 S=3")
  if (/download|file|path|template|render|preview|export/i.test(combined))
    add("file/read/render surface", "path/template/render canary differential | V=4 C=2 IG=3 R=2 SD=0 S=3")
  if (
    runtime.domSinks.length ||
    runtime.postMessage.length ||
    /<script|data-reactroot|id=["']root|__NEXT_DATA__|vite|webpack|nuxt|vue|angular/i.test(combined)
  )
    browserNeeds.push("SPA/DOM/network route discovery")
...
export default tool({
  description:
    "CTF Web black-box map v8: safe low-volume discovery for URL-only web challenges. Builds tech fingerprint, surface map, static browser-runtime signals, state model, and a ranked attack queue seed without payload spraying.",
  args: {
    ...
    mode: tool.schema
      ...
      .optional()
      .describe(
        "light | browser | deep. Default light. browser/deep fetch more same-origin JS and model runtime/state signals.",
      ),
    ...
    maxRequests: tool.schema
      .number()
      .optional()
      .describe("Maximum safe discovery requests. Default depends on mode; hard cap 40."),
    maxScripts: tool.schema
      .number()
      .optional()
      .describe("Maximum same-origin JS files to inspect. Default depends on mode; hard cap 20."),
    extraPaths: tool.schema
      .string()
      .optional()
      .describe("Optional newline/comma-separated same-origin paths to include in the safe discovery budget."),
  },
  async execute(args) {
    ...
    const runtime = {
      ...
      csp: unique(
        headersLines.filter((x) => /content-security-policy/i.test(x)).map((x) => x.slice(0, 220)),
        20,
      ),
    ...
    const apiCandidates = unique(
      allRoutes.filter((x) => /\/api|graphql|openapi|swagger|callback|webhook/i.test(x)),
      80,
    )
    const adminCandidates = unique(
      allRoutes.filter((x) => /admin|manage|dashboard|internal|actuator/i.test(x)),
      50,
    )
    const uploadCandidates = unique(
      [...formsUnique, ...allRoutes].filter((x) => /upload|import|avatar|file|multipart/i.test(x)),
      50,
    )
    const debugCandidates = unique(
      [...allRoutes, ...statusMap, ...jsFindings].filter((x) =>
        /debug|trace|sourceMappingURL|\.map|\.git|\.env|actuator|stack/i.test(x),
      ),
      50,
    )
    const authCandidates = unique(
      [...formsUnique, ...allRoutes, ...runtime.fetchTargets].filter((x) =>
        /login|logout|register|reset|session|csrf|xsrf|oauth|saml|jwt|token|profile|user/i.test(x),
      ),
      80,
    )
    const stateModel = {
      roles: unique(
        [...allRoutes, ...(allText.match(/admin|moderator|user|guest|owner|role|tenant/gi) ?? [])].map((x) =>
          String(x).slice(0, 120),
        ),
        50,
      ),
      loginRegisterReset: unique(
        [...authCandidates].filter((x) => /login|register|reset|logout|session/i.test(x)),
        50,
      ),
      objectIdPatterns: unique(
        Array.from(
          allText.matchAll(/\b(?:id|userId|uid|orderId|postId|fileId|tenantId|teamId)["'\s:=]+([A-Za-z0-9_-]{1,80})/g),
          (m) => `${m[0].slice(0, 80)}`,
        ),
        80,
      ),
      workflowEdges: unique(
        [...formsUnique, ...runtime.fetchTargets].filter((x) =>
          /create|update|delete|approve|submit|publish|share|report|preview|export|import|checkout|pay/i.test(x),
        ),
        80,
      ),
    }
    ...
    const flagHits = unique(
      Array.from(allText.matchAll(/[A-Za-z0-9_@.-]{2,32}\{[^\r\n}]{1,200}\}/g), (m) => m[0]),
      20,
    )

    const recommendations: string[] = []
    if (flagHits.length) recommendations.push("verify flag-like hit immediately; do not broaden exploration")
    if (jsFindings.some((x) => /source_map_hint/.test(x)))
      recommendations.push("fetch source maps before fuzzing; then pivot to source-guided audit bridge")
    if (browserNeeds.length || mode !== "light")
      recommendations.push(
        "run ctf-web-runtime-map or browser MCP for the listed browser-specific need; keep curl as final reproduction when possible",
      )
    if (queue.length) recommendations.push("feed attack_queue_seed into ctf-decision-state rank before focused probes")
    if (apiCandidates.length)
      recommendations.push("use ctf-web-diff-probe on the most interesting API endpoint; one variable only")
    if (authCandidates.length)
      recommendations.push("use ctf-web-authz-matrix when two accounts or object IDs are available")
    if (!queue.length)
      recommendations.push(
        "next cheapest orthogonal checks: compare curl vs browser-rendered DOM, inspect JS-discovered routes, then one-variable oracle probes",
      )
