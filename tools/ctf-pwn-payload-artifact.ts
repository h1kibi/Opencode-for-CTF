...
function parseBytes(args: any) {
  ...
  if (args.repeatByte !== undefined && args.length) return Buffer.alloc(Number(args.length), Number(args.repeatByte) & 0xff)
  return Buffer.alloc(0)
}

function qwordViews(buf: Buffer, offsets: number[]) {
  return offsets.filter((o) => o >= 0 && o < buf.length).map((o) => {
...
export default tool({
  description: "CTF PWN payload artifact helper: write/patch binary payloads safely and report length, sha256, xxd preview, and qword views.",
  args: {
    ...
    fromFile: tool.schema.string().optional().describe("Optional existing workspace-relative file to load before patches."),
  ...
  async execute(args, context) {
    let buf = args.fromFile ? await readFile(resolveInsideWorkspace(context.directory, args.fromFile)) : parseBytes(args)
    ...
    for (const p of patches) {
      ...
      else if (p.u64le !== undefined) { pb = Buffer.alloc(8); pb.writeBigUInt64LE(BigInt(p.u64le)) }
      else if (p.u32le !== undefined) { pb = Buffer.alloc(4); pb.writeUInt32LE(Number(p.u32le) >>> 0) }
    ...
    const offsets = String(args.qwordOffsets || "0,8,16,24,32,40,48,56,64")
      .split(/[\r\n,]+/).map((x) => x.trim()).filter(Boolean).map((x) => Number(x.startsWith("0x") ? parseInt(x, 16) : parseInt(x, 10))).filter((x) => Number.isFinite(x))
