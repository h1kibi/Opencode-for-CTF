...
async function fileMeta(contextDir: string, label: string, input?: string) {
  ...
  try {
    ...
    let buildId = ""
    const r = await safeExec("readelf", ["-n", abs], { cwd: path.dirname(abs), timeoutMs: 4000, maxBuffer: 512 * 1024 })
    buildId = r.output.match(/Build ID:\s*([0-9a-f]+)/i)?.[1] || ""
    return {
      label,
      path: path.relative(contextDir, abs).replace(/\\/g, "/"),
      abs_path: abs,
      exists: true,
      size: buf.length,
      sha256,
...
export default tool({
  description:
    "CTF PWN runtime lock: create a hard identity card for binary/loader/libc/patched/docker/remote runtime and save it as a reusable profile.",
  args: {
    ...
    patchedBinary: tool.schema
      .string()
      .optional()
      .describe("Workspace-relative patched binary if different from original."),
    originalBinary: tool.schema
      .string()
      .optional()
      .describe("Workspace-relative original binary if binary is patched."),
    ...
    patched: tool.schema
      .boolean()
      .optional()
      .describe("Whether the executed binary is patched. Inferred from patchedBinary when omitted."),
    lockId: tool.schema
      .string()
      .optional()
      .describe("Optional stable lock id. Default derived from binary/libc/ld/docker identity."),
  ...
  async execute(args, context) {
    ...
    const remote = {
      enabled: Boolean(args.remoteHost || args.remotePort),
    ...
    const idMaterial = JSON.stringify({
      b: (binary as any).sha256,
      l: (loader as any).sha256,
      c: (libc as any).sha256,
      patched,
      docker,
      remote,
    })
    ...
    const explicitLoaderArgv = args.loader
      ? [
          path.posix.join(docker.container_mount_root, String(args.loader).replace(/\\/g, "/")),
          "--library-path",
          path.posix.dirname(
            path.posix.join(docker.container_mount_root, String(args.libc || args.loader).replace(/\\/g, "/")),
          ),
          path.posix.join(docker.container_mount_root, String(args.binary).replace(/\\/g, "/")),
        ]
      : []
    const payload = {
