...
export default tool({
  description:
    "CTF Python parallel runner: write a real .py file and execute multiprocessing-safe Python on Windows with explicit __main__ guard checks.",
  args: {
    ...
    scriptPath: tool.schema
      .string()
      .optional()
      .describe("Workspace-relative script path. Default work/python-parallel/run.py."),
  ...
  async execute(args, context) {
    ...
    if (process.platform === "win32" && !hasMainGuard(args.code)) {
      throw new Error(
        "Windows multiprocessing-safe execution requires `if __name__ == '__main__':` in the script. Add the guard and retry.",
      )
    }
    ...
    const { stdout, stderr, ok } = await safeExecWithStreams(py.cmd, [...py.baseArgs, scriptPath, ...argv], {
      cwd: workdir,
      timeoutMs,
    ...
    if (!ok) throw new Error(stderr.trim() || "python execution failed")
    const payload = {
      ok: true,
