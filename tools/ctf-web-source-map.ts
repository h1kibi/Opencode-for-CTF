...
const SOURCE_EXT = /\.(py|js|mjs|cjs|ts|tsx|jsx|php|java|kt|go|rb|rs|html|hbs|ejs|jinja|twig|jsp|xml|json|ya?ml|toml|ini|properties|env|conf|cfg)$/i
const MANIFEST = /(^|[\\/])(package\.json|requirements\.txt|pyproject\.toml|pom\.xml|build\.gradle|go\.mod|composer\.json|cargo\.toml|Dockerfile|docker-compose\.ya?ml|compose\.ya?ml|\.env(\.example|\.sample)?|config\.(js|ts|json|php|py|ya?ml))$/i
...
const ROUTE_PATTERNS: Array<[string, RegExp]> = [
  ...
  ["spring", /@(GetMapping|PostMapping|RequestMapping|PutMapping|DeleteMapping)\((?:value\s*=\s*)?["'`]([^"'`]+)["'`]/g],
...
const INPUT_PATTERNS: Array<[string, RegExp]> = [
  ...
  ["node req query/body/params/cookies/files/headers", /req\.(query|body|params|cookies|files|headers)|request\.(query|body|params)/g],
  ["php superglobal", /\$_(GET|POST|REQUEST|COOKIE|FILES|SERVER)\[/g],
  ["java request param/header/session", /getParameter\(|getHeader\(|getSession\(|@RequestParam|@RequestBody|@PathVariable/g],
...
const SINK_PATTERNS: Array<[string, RegExp]> = [
  ["command execution", /\b(eval|exec|system|popen|shell_exec|passthru|subprocess\.|os\.system|child_process|execSync|spawn\(|ProcessBuilder|Runtime\.getRuntime|Command::new)\b/g],
  ["template injection", /render_template_string|Template\(|jinja2|twig|handlebars|ejs\.render|pug\.render|res\.render/g],
  ["SQL/string query", /\b(select|insert|update|delete)\b[\s\S]{0,80}(\+|%|format\(|f["'`]|\$\{|\.format)|execute\([^,)]*\+|rawQuery|createQuery|Statement\./gi],
  ["deserialization", /pickle\.loads|yaml\.load|unserialize\(|ObjectInputStream|readObject\(|JSON\.parse\(|marshal\.loads|phpggc/g],
  ["file/path", /readFile|writeFile|createReadStream|send_file|open\(|FileInputStream|Files\.read|Path\.join|\.\.[\\/]|download|upload|move_uploaded_file/g],
...
const SECRET_PATTERNS: Array<[string, RegExp]> = [
  ["hardcoded secret/env", /(SECRET_KEY|JWT_SECRET|API_KEY|TOKEN|PASSWORD|PASSWD|FLAG|PRIVATE_KEY|SESSION_SECRET)\s*[:=]\s*["'`]?([^"'`\n#]+)/gi],
  ["flag-like", FLAG_RE],
...
function recommend(routeHits: string[], inputHits: string[], sinkHits: string[], secretHits: string[]) {
  const joined = `${routeHits.join("\n")}\n${inputHits.join("\n")}\n${sinkHits.join("\n")}\n${secretHits.join("\n")}`.toLowerCase()
  const out: string[] = []
  if (/flag-like|flag\{|hardcoded secret|secret_key|jwt_secret/.test(joined)) out.push("verify exposed flag/secret first; this is usually faster than exploitation")
  if (/template injection|render_template_string|ejs\.render|twig/.test(joined)) out.push("test SSTI with one low-risk arithmetic payload")
  if (/sql\/string query|statement|rawquery|select/.test(joined)) out.push("test SQLi/auth bypass before full fuzzing")
  if (/command execution|subprocess|child_process|runtime\.getruntime|processbuilder/.test(joined)) out.push("trace user input to command sink; prefer one controlled command proof")
  if (/deserialization|pickle|unserialize|objectinputstream|yaml\.load/.test(joined)) out.push("inspect signing/secret and gadget surface before payload generation")
  if (/file\/path|upload|download|send_file|readfile|writefile/.test(joined)) out.push("check path traversal/upload/file-write matrix before brute forcing")
  if (/ssrf|url fetch|requests\.|axios|fetch\(/.test(joined)) out.push("check SSRF/internal route path if user controls URL")
...
export default tool({
  description: "CTF web source map: compact static route/input/sink/secret map for source-code Web challenges before manual review or fuzzing.",
  args: {
  ...
  async execute(args, context) {
    ...
    for (const f of filePaths) {
      try {
        ...
        files.push({ file: f.file, rel: f.rel, score: scoreRel(f.rel), text: buf.subarray(0, maxBytesPerFile).toString("utf8") })
      } catch {
    ...
    const manifests = files.filter((f) => MANIFEST.test(f.rel)).slice(0, 30).map((f) => f.rel)
    const entrypoints = files.filter((f) => scoreRel(f.rel) >= 22 && !MANIFEST.test(f.rel)).slice(0, 40).map((f) => f.rel)
