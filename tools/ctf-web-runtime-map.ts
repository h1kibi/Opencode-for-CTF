...
function extractInteractiveElements(text: string) {
  const buttons = Array.from(text.matchAll(/<(?:button|a|input)\b([^>]{0,500})>/gi), (m) =>
...
function extractNetwork(text: string) {
  const fetchTargets = [
    ...
    ...Array.from(
      text.matchAll(/\baxios\.(get|post|put|patch|delete)\(\s*["'`]([^"'`]+)["'`]/g),
      (m) => `axios.${m[1]} ${m[2]}`,
    ),
    ...Array.from(
      text.matchAll(/\.open\(\s*["'`](GET|POST|PUT|PATCH|DELETE)["'`]\s*,\s*["'`]([^"'`]+)/g),
      (m) => `xhr.${m[1]} ${m[2]}`,
    ),
...
function extractSecurityHeaders(headers: string[]) {
  return headers
    .filter((x) =>
      /content-security-policy|x-frame-options|permissions-policy|referrer-policy|cross-origin|strict-transport-security/i.test(
        x,
      ),
    )
    .map((x) => x.slice(0, 260))
...
function extractDom(text: string) {
  ...
  return {
    sinks: unique(
      sinkTerms.filter((x) => text.includes(x)),
      80,
    ),
    sources: unique(
      sources.filter((x) => text.includes(x)),
      80,
    ),
    postMessage: unique(
      [
        ...
        /targetOrigin\s*[:=]\s*["']\*["']|postMessage\s*\([\s\S]{0,120}["']\*["']/.test(text)
          ? "wildcard targetOrigin candidate"
          : "",
      ],
      20,
    ),
    serviceWorkers: unique(
...
function adminBotProfile(text: string, headers: string[]) {
  ...
  const signals = unique(
    [
      ...Array.from(
        combined.matchAll(
          /\b(report|admin bot|bot|headless|puppeteer|selenium|phantomjs|share|preview|contact|feedback)\b/gi,
        ),
        (m) => m[0].toLowerCase(),
      ),
    ],
...
export default tool({
  description:
    "CTF Web runtime map v8: static/browser-adjacent runtime intelligence from HTML and same-origin JS. Extracts network calls, forms, storage keys, cookie/security headers, DOM sinks, postMessage, service workers, and admin-bot signals without using personal browser history/bookmarks.",
  args: {
    ...
    maxScripts: tool.schema
      .number()
      .optional()
      .describe("Maximum same-origin JS files to inspect. Default 12, hard cap 24."),
    maxBytesPerScript: tool.schema
      .number()
      .optional()
      .describe("Maximum bytes per script. Default 600000, hard cap 1200000."),
  },
  async execute(args) {
    ...
    try {
      ...
      status.push(
        `${base.pathname || "/"}: ${r.status} ${r.headers.get("content-type") ?? ""}${body.truncated ? " truncated" : ""}`,
      )
    ...
    for (const script of unique(scripts, maxScripts)) {
      try {
        ...
        texts.push(body.text)
        status.push(
          `${u.pathname}: ${r.status} ${r.headers.get("content-type") ?? ""}${body.truncated ? " truncated" : ""}`,
        )
      } catch (err) {
    ...
    const next: string[] = []
    if (network.fetchTargets.length || network.routes.length)
      next.push("promote high-value XHR/API routes into ctf-decision-state hypotheses")
    ...
    if (secHeaders.some((x) => /content-security-policy/i.test(x)))
      next.push("record CSP before choosing exfil/control plane")
    ...
    return [
      ...
      ...(next.length
        ? next.map((x) => `- ${x}`)
        : ["- run ctf-web-diff-probe on the highest-value endpoint or return to ctf-decision-state rank"]),
      "errors:",
