...
const chains: Chain[] = [
  {
    ...
    terms: ["actuator", "management.endpoint", "management.endpoints", "spring-boot-starter-actuator", "mappings", "heapdump"],
    value: 5, confidence: 3, infoGain: 5, cost: 1, risk: 1, stateDamage: 1, stability: 5,
    ...
    firstProbe: "Fetch actuator index or mappings/env availability only; record status, content-type, length, and auth differential.",
    ...
    closure: ["config/env key read", "mappings to hidden route", "heapdump/logfile to secret/flag", "H2/Swagger follow-up"],
  },
  {
    ...
    value: 5, confidence: 3, infoGain: 5, cost: 1, risk: 2, stateDamage: 1, stability: 4,
    ...
    closure: ["read flag table", "read users/admin", "read config from DB", "H2 parser chain only if route gates exist"],
  },
  {
    ...
    value: 5, confidence: 3, infoGain: 5, cost: 2, risk: 2, stateDamage: 1, stability: 4,
  ...
  },
  {
    ...
    terms: ["thymeleaf", "spel", "ExpressionParser", "SpelExpressionParser", "Freemarker", "FreeMarker", "Velocity", "ModelAndView", "setViewName", "template-expression-candidate"],
    value: 5, confidence: 3, infoGain: 4, cost: 2, risk: 2, stateDamage: 1, stability: 4,
    ...
    firstProbe: "Use one harmless engine-specific expression or template-resolution marker against the controlled field.",
    ...
    closure: ["read flag/config via Spring/JDK file utilities", "admin preview render", "command only after file-read closure fails"],
  },
  {
    id: "java-upload-writeback",
    title: "Upload/file write to served readback or JSP closure",
    family: "file-write",
    terms: ["MultipartFile", "transferTo", "FileOutputStream", "Files.write", "getRealPath", "upload-archive-sink", "jsp"],
    value: 5, confidence: 3, infoGain: 4, cost: 2, risk: 3, stateDamage: 3, stability: 3,
    requires: ["write/upload route", "controlled name/content/path", "canary create/readback", "served/reload behavior for JSP"],
  ...
  },
  {
    ...
    value: 5, confidence: 2, infoGain: 4, cost: 3, risk: 3, stateDamage: 1, stability: 3,
    requires: ["Shiro present", "rememberMe parser or filter-chain evidence", "key/config/classpath or auth route oracle"],
    produces: ["auth bypass", "deserialization primitive", "admin route"],
    firstProbe: "Confirm rememberMe/deleteMe or filter-chain behavior before gadgets; map key/classpath if parser route exists.",
  ...
  },
  {
    ...
    terms: ["ObjectInputStream", "readObject", "XMLDecoder", "fastjson", "parseObject", "ObjectMapper", "activateDefaultTyping", "enableDefaultTyping", "XStream", "Yaml", "rO0AB", "aced0005", "@type"],
    value: 5, confidence: 2, infoGain: 4, cost: 3, risk: 3, stateDamage: 1, stability: 3,
    requires: ["parser call", "controlled bytes/string/body/cookie/file", "type/parser config", "classpath or feature", "oracle"],
  ...
  },
  {
    ...
    family: "authz-logic",
    terms: ["SecurityFilterChain", "antMatchers", "requestMatchers", "permitAll", "hasRole", "HandlerInterceptor", "excludePathPatterns", "@RequestBody", "@ModelAttribute", "isAdmin", "role"],
    value: 5, confidence: 3, infoGain: 5, cost: 2, risk: 1, stateDamage: 2, stability: 4,
...
function rankScore(chain: Chain, hits: number, mode: string) {
  const base = (2 * chain.value + chain.confidence + chain.infoGain + chain.stability) - (chain.cost + chain.risk + chain.stateDamage)
  const hardBonus = mode === "hard" && ["deserialization", "shiro", "authz-logic"].includes(chain.family) ? 2 : 0
  return base + hits * 4 + hardBonus
}

export default tool({
  description: "CTF Java Web chain planner: convert Java map/config/dep/source-slice evidence into ranked exploit-chain candidates with gates, first probes, and closure paths.",
  args: {
    evidence: tool.schema.string().describe("Compact Java Web evidence from ctf-java-map/config/source-slice/dep-risk/blackbox output."),
  ...
  async execute(args) {
    ...
    return [
      ...
      "",
      "## Ranked Chains",
      ...(ranked.length ? ranked.map(({ chain, hits, score }, i) => [
        `### #${i + 1} ${chain.id}`,
        `title: ${chain.title}`,
        `family: ${chain.family}`,
