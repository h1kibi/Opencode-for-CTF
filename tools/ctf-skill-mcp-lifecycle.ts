...
import {
  ensureSkillMcpLeases,
  listSkillMcpLeases,
  releaseSkillMcpLeases,
} from "../src/skill-mcp-manager.ts"

export default tool({
  description: "CTF skill-embedded MCP lifecycle: activate, release, or inspect dynamic MCP leases for skills on a per-session basis.",
  args: {
    operation: tool.schema.string().describe("activate | release | list | release_all_session"),
    skillName: tool.schema.string().optional().describe("Skill name for activate/release, e.g. ctf-web-java or ctf-seckb."),
  ...
  async execute(args, context) {
    ...
    return args.jsonOnly ? JSON.stringify(state, null, 2) : [
      ...
      ...state.leases.map((lease) => `- session=${lease.sessionID} skill=${lease.skillName} server=${lease.serverName} connected=${lease.connected} idle_release=${lease.disconnectWhenIdle ?? true}`),
    ].join("\n")
  },
})
