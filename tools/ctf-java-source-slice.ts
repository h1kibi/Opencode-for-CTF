...
const inputRe =
  /@(RequestParam|PathVariable|RequestBody|RequestHeader|CookieValue|ModelAttribute)|HttpServletRequest|MultipartFile|Part\b|getParameter\(|getHeader\(|getCookies\(/i
const sinkFamilies: { name: string; re: RegExp; reference: string }[] = [
  {
    name: "sql",
    re: /\$\{|Statement\.|PreparedStatement\.|JdbcTemplate|createQuery\(|createNativeQuery\(|@Select\(|@Update\(|@Insert\(|@Delete\(/i,
    reference: "java-sql-mybatis.md",
  },
  {
    name: "file-upload-path",
    re: /FileInputStream|FileOutputStream|Files\.read|Files\.write|Paths\.get|Path\.of|MultipartFile|transferTo\(|ZipInputStream|ZipEntry|ClassPathResource|ServletContext\.getResource|getRealPath/i,
    reference: "java-file-upload-path.md",
  },
  {
    name: "template-expression",
    re: /TemplateEngine|FreeMarker|Freemarker|VelocityEngine|ExpressionParser|SpelExpressionParser|Ognl|ModelAndView|setViewName|InternalResourceViewResolver|\.ftl|\.vm|\.jsp/i,
    reference: "java-template-expression.md",
  },
  {
    name: "deser-parser",
    re: /ObjectInputStream|readObject|XMLDecoder|JSON\.parseObject|parseObject\(|ObjectMapper|enableDefaultTyping|activateDefaultTyping|new\s+Yaml\(|XStream/i,
    reference: "java-deser-shiro-fastjson.md",
  },
  {
    name: "xxe",
    re: /DocumentBuilderFactory|SAXParserFactory|XMLInputFactory|SAXReader|TransformerFactory|setFeature\(/i,
    reference: "java-xxe-ssrf.md",
  },
  {
    name: "ssrf",
    re: /RestTemplate|HttpClient|WebClient|OkHttpClient|URL\(|URI\(|openConnection/i,
    reference: "java-xxe-ssrf.md",
  },
...
function methodAt(content: string, index: number) {
  ...
  const matches = Array.from(
    before.matchAll(/(?:public|private|protected)?\s*(?:static\s+)?[\w<>\[\], ?]+\s+(\w+)\s*\([^)]*\)/g),
  )
...
export default tool({
  description:
    "CTF Java source slice: scan Java/JSP/XML source for route/input/sink co-location, source-to-sink slices, reference dispatch, and first safe checks for Java Web challenges.",
  args: {
    target: tool.schema.string().describe("Java Web source/extracted directory to slice"),
    focus: tool.schema
      .string()
      .optional()
      .describe("Optional focus keyword such as route path, class, sink family, parameter, or dependency clue"),
  },
  async execute(args, context) {
    ...
    for (const file of files.slice(0, 800)) {
      ...
      const injected = Array.from(
        content.matchAll(
          /(?:@Autowired\s+|private\s+final\s+|private\s+)([A-Z]\w*(?:Service|Mapper|Repository|Dao|DAO|Client|Util|Helper))\s+(\w+)/g,
        ),
      ).slice(0, 20)
      for (const m of injected) xrefs.push(`xref-field: ${rel} class=${cls} type=${m[1]} name=${m[2]}`)
      const calls = Array.from(content.matchAll(/\b(\w+)\.(\w+)\s*\(/g))
        .filter((m) => !["System", "String", "Math", "Objects", "Collections", "Arrays"].includes(m[1]))
        .slice(0, 40)
      for (const m of calls) xrefs.push(`xref-call: ${rel} class=${cls} ${m[1]}.${m[2]}(...)`)
      if (/namespace\s*=|<mapper\b|@(Mapper|Repository)\b/i.test(content))
        xrefs.push(`xref-mapper: ${rel} class=${cls}`)
    ...
    const queue: string[] = []
    if (routes.length && sinks.length)
      queue.push(
        "Route/input/sink evidence exists: build Java Constraint Equation and choose the highest-value co-located or xref-able sink",
      )
    if (sinks.some((x) => x.includes("sql")))
      queue.push("SQL/MyBatis: inspect mapper/service reachability and binding type before payloads")
    if (sinks.some((x) => x.includes("file-upload-path")))
      queue.push("File/upload/path: map base dir, normalization, extension checks, canary behavior")
    if (sinks.some((x) => x.includes("template-expression")))
      queue.push("Template/expression: identify engine and context before expression marker")
    if (sinks.some((x) => x.includes("deser-parser")))
      queue.push("Deser/parser: verify parser/config/data shape before gadgets")
    if (sinks.some((x) => x.includes("xxe") || x.includes("ssrf")))
      queue.push("XXE/SSRF: identify parser/fetcher and validator before one safe differential")
    if (!queue.length)
      queue.push("No strong slice found; run ctf-java-map/config/dep-risk or focus on a route/sink keyword")
