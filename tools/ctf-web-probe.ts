...
export default tool({
  description:
    "CTF web probe: fetch one explicit URL plus robots.txt and sitemap.xml, summarize status, headers, cookies, forms, links, scripts, and likely bug-class hints.",
  args: {
  ...
  async execute(args) {
    ...
    const forms = Array.from(text.matchAll(/<form\b[\s\S]*?<\/form>/gi), (m) =>
      m[0].replace(/\s+/g, " ").slice(0, 300),
    ).slice(0, 20)
    const links = Array.from(
      new Set(Array.from(text.matchAll(/href=["']([^"']+)/gi), (m) => sameOrigin(base, m[1])).filter(Boolean)),
    ).slice(0, 50)
    const scripts = Array.from(
      new Set(Array.from(text.matchAll(/src=["']([^"']+\.js[^"']*)/gi), (m) => sameOrigin(base, m[1])).filter(Boolean)),
    ).slice(0, 50)
    ...
    if (/ueditor|editor|filemanager|uploadfile|listfile|controller/i.test(text))
      hints.add("editor/file-write candidates")
    if (/debug.?true|traceback|stack trace|werkzeug|django|flask|express error/i.test(text))
      hints.add("debug/source-leak")
    ...
    if (hints.has("admin-bot/browser")) {
      ...
      attackQueueSeed.push(
        "admin-bot path | expected primitive: privileged browser execution | value 3 | cost 3 | risk 3 | stability 3",
      )
    ...
    if (hints.has("editor/file-write candidates")) {
      ...
      attackQueueSeed.push(
        "editor/file-write path | expected primitive: server-side file write | value 5 | cost 2 | risk 4 | stability 4",
      )
    ...
    if (hints.has("debug/source-leak")) {
      ...
      attackQueueSeed.push(
        "debug/source-leak path | expected primitive: path/config/source disclosure | value 4 | cost 1 | risk 1 | stability 5",
      )
    ...
    if (hints.has("java-web")) {
      ...
      recommendedNext.push(
        "use Java Web source/framework triage if source, jar, war, pom.xml, or stack traces are available",
      )
    }
    ...
    return [
      ...
      ...(recommendedNext.length
        ? recommendedNext.map((x) => `- ${x}`)
        : ["- build route/input/auth map and attack queue"]),
      "risk warnings:",
