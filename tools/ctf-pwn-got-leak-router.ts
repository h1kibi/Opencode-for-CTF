...
export default tool({
  description:
    "CTF pwn got leak router: decide whether GOT contents can be reused as strings, pointers, or indirect parameters and suggest short leak routes such as rbp-k to got entry pivots.",
  args: {
    evidence: tool.schema
      .string()
      .describe(
        "Source snippet, decompilation, disassembly, or notes involving printf/puts/fgets/read and GOT/global pointers.",
      ),
  ...
  async execute(args) {
    ...
    if (
      has(lower, /lea\s+rax,\s*\[rbp-0x[0-9a-f]+\]/) &&
      has(lower, /mov\s+rdi,\s*rax/) &&
      has(lower, /printf@plt|puts@plt/)
    )
      signals.push("rbp-relative-pointer-fed-into-print")
    if (
      has(lower, /lea\s+r(ax|di|si|dx|cx|8|9),\s*\[rbp-0x[0-9a-f]+\]/) &&
      has(lower, /mov\s+rdi,\s*r(ax|di|si|dx|cx|8|9)/) &&
      has(lower, /printf@plt|puts@plt|call.*printf|call.*puts/)
    )
      signals.push("frame-indexed-first-argument-callsite")
    if (has(lower, /leave\s*;?\s*ret|\bleave\b/) && has(lower, /rbp-0x[0-9a-f]+/))
      signals.push("leave-ret-pseudostack-mid-function-risk")
    if (has(lower, /call.*<(printf|puts)@plt>|call.*printf|call.*puts/) && has(lower, /rbp-0x[0-9a-f]+/))
      signals.push("possible-mid-function-reentry-callsite")
    if (has(lower, /@got/) || has(lower, /got\.plt|\.got/)) signals.push("got-surface-present")
    if (has(lower, /printf@plt/) && has(lower, /mov\s+rdi,\s*rax|mov\s+rsi,\s*rax/))
      signals.push("printable-pointer-consumer")
    ...
    if (signals.includes("rbp-relative-pointer-fed-into-print") && signals.includes("got-surface-present")) {
      ...
      routes.push(
        "control rbp so rbp-k lands on a GOT entry or nearby global pointer and let the existing print path leak it",
      )
    }
    if (signals.includes("string-or-pointer-output-path")) {
      routes.push(
        "classify whether the consumer dereferences as string, prints raw pointer, or forwards as indirect argument before assuming classic ret2libc",
      )
    }
    if (signals.includes("frame-indexed-first-argument-callsite")) {
      routes.push(
        "treat this as frame-indexed callsite reuse: set saved rbp so rbp-k points at GOT/global/string, then re-enter only after rdi/rsi/rdx state is verified",
      )
    }
    if (signals.includes("leave-ret-pseudostack-mid-function-risk")) {
      routes.push(
        "use ctf-pwn-stage-harness preset=leave_ret_pseudostack_midcall and ctf-pwn-stage-delta-runner before promoting ordinary ROP",
      )
    }
    const frameOff = text.match(/\[rbp\s*-\s*0x([0-9a-f]+)\]/i)?.[1]
    const k = frameOff ? `0x${parseInt(frameOff, 16).toString(16)}` : "k"
    const gotNames = ["printf", "puts", "read", "write", "__libc_start_main"].filter(
      (name) => new RegExp(name, "i").test(text) || signals.includes("got-surface-present"),
    )
    ...
    const primitiveProbe =
      signals.includes("frame-indexed-first-argument-callsite") || signals.includes("FRAME_INDEXED_PRINTF_LEAK")
        ? {
            ...
            candidate_targets: candidateTargets.length
              ? candidateTargets
              : [{ target: "printf@got", rbp: `printf@got + ${k}`, why: "default first GOT leak target" }],
