...
export default tool({
  description:
    "CTF Web fuzz plan: ffuf/gobuster-inspired planner that creates a low-noise fuzzing strategy, baseline filters, stop conditions, and ranking rules. It does not execute fuzzing.",
  args: {
    target: tool.schema.string().describe("Target URL or request template."),
    dimension: tool.schema
      .string()
      .describe("path | vhost | param | header | method | content-type | value | api-route"),
  ...
  async execute(args) {
    ...
    const filters: string[] = [
      "calibrate one known-miss baseline first",
      "filter by stable status+length+words, not status alone",
      "manually inspect top anomalies before continuing",
    ]
    const stops: string[] = [
      "stop after first new route class with a stronger direct hypothesis",
      "stop if 3 consecutive result groups add no new differential",
      "stop before state-changing endpoints unless selected by ctf-decision-state",
    ]
    const commandShape: string[] = []
    const wordlists: string[] = []
    const rank: string[] = [
      "new status not seen in baseline",
      "large body-length delta",
      "new content-type",
      "auth boundary change",
      "route keyword intersects flag/admin/debug/upload/download/report/api",
    ]
    if (dimension === "path" || dimension === "api-route") {
      ...
      wordlists.push(
        "small CTF common paths",
        "framework-specific paths from fingerprint",
        "JS-discovered route stems first",
      )
    } else if (dimension === "vhost") {
      commandShape.push(
        "ffuf -u http://<host>/ -H 'Host: FUZZ.<host>' -w <tiny-vhost-list> -mc all -fs <baseline-size>",
      )
      ...
      commandShape.push(
        "compare application/json, x-www-form-urlencoded, multipart/form-data, text/plain with one stable body",
      )
    ...
    return [
      ...
      ...(wordlists.length
        ? wordlists.map((x) => `- ${x}`)
        : ["- no wordlist needed; use enumerated one-variable comparisons"]),
      "baseline_filters:",
      ...filters.map((x) => `- ${x}`),
      "ranking_rules:",
      ...rank.map((x) => `- ${x}`),
