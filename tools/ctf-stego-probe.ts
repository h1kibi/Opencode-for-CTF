...
function trailingDataHint(fullOrSample: Buffer, size: number) {
  ...
  if (pngIdx >= 0 && pngIdx + pngEnd.length < fullOrSample.length)
    hints.push(`png trailing data after sample offset ${pngIdx + pngEnd.length}`)
  const jpgIdx = fullOrSample.lastIndexOf(Buffer.from([0xff, 0xd9]))
  if (jpgIdx >= 0 && jpgIdx + 2 < fullOrSample.length)
    hints.push(`jpeg trailing data after sample offset ${jpgIdx + 2}`)
  const eocdIdx = fullOrSample.lastIndexOf(Buffer.from([0x50, 0x4b, 0x05, 0x06]))
  if (eocdIdx >= 0 && eocdIdx + 22 < fullOrSample.length)
    hints.push(`zip-like trailing/overlay data around sample offset ${eocdIdx + 22}`)
  if (size > fullOrSample.length)
    hints.push("large file sampled only; run binwalk/exiftool or targeted extraction if hints look promising")
  return hints
}
...
export default tool({
  description:
    "CTF stego/media probe: fast metadata, signature, trailing-data, embedded-archive, strings, binwalk/zsteg availability hints without slow brute force.",
  args: {
  ...
  async execute(args, context) {
    ...
    const secretStrings = Array.from(
      new Set(strings.filter((s) => /flag|secret|password|token|key|ctf|base64|hidden/i.test(s))),
    ).slice(0, 80)
    const signatureHits = findSignatures(sample)
    const trailingHints = trailingDataHint(sample, stat.size)

    const fileR = await safeExec("file", [target], cwd)
    const fileOut = fileR.output
    const exifR = await safeExec("exiftool", [target], cwd, 8000)
    const exifOut = exifR.output
    const binwalkR = await safeExec("binwalk", [target], cwd, 8000)
    const binwalkOut = binwalkR.output
    const zstegOut = /\.(png|bmp)$/i.test(target)
      ? (await safeExec("zsteg", [target], cwd, 8000)).output
      : "skipped_non_png_bmp"
    ...
    if (signatureHits.some((x) => /zip|rar|7z|gzip|pdf/.test(x)))
      recommended.push("embedded file signature found: carve/extract that offset")
    if (!binwalkOut.startsWith("<failed") && /Zip|RAR|7-zip|gzip|PNG|JPEG|compressed|filesystem/i.test(binwalkOut))
      recommended.push("binwalk shows embedded/compressed object; extract only relevant offsets")
    if (!zstegOut.startsWith("<failed") && !/nothing|skipped/i.test(zstegOut))
      recommended.push("zsteg produced output; verify high-signal rows first")
    if (!recommended.length)
      recommended.push("metadata/strings/signatures first; only then consider LSB or password guessing")
