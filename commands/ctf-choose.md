...
- Recommend `ctf-master` when source, archives, bytecode, Docker/config, multi-role/authz/workflow, race, deserialization, parser confusion, or other real branch complexity is present.
- Bias toward `ctf-fast` for fresh easy/medium Web tasks with obvious primitives.
- Bias toward `ctf-master` when the user already reports being stuck, when the challenge has multiple artifacts/services, or when a clean exploit path is not obvious.
